...
def get_rate(net_card):
    ...
    if net_card is None:  # 抓取全部网卡的速度
        net_cards = net_io_counters(pernic=True).keys() #Return network I/O statistics as a namedtuple
    else:
        net_cards.append(net_card)
    for card in net_cards:
        # 上一秒收集的数据
        info = get_net_flow(card) #获得收发数据包的字节数和包数信息【bytes_sent、bytes_recv、packets_sent、packets_recv】
        for i in range(4):
            old[i] += info[i]
    time.sleep(1)
    # 当前所收集的数据
    ...
    info = []
    for i in range(4):
        info.append(new[i] - old[i]) # 这一秒的速度和上一秒的速度之差
    return info
